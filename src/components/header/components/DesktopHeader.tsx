    ...
    <div className="hidden md:flex justify-between items-center w-full bg-gradient-to-r from-[#FFA163] to-[#FFCF00] px-2.5 max-w-[1440px] mx-auto sticky top-0 z-10">
      <div className="flex items-center">
        ...
        <OldButtonV2 onClick={() => {}} className="bg-[#C0C0C0] mr-2">
          <div
            className="flex flex-col w-28 py-1 justify-center items-center italic font-bold text-[15px] gap-0 "
        ...
        <OldButtonV2 onClick={() => openModal(ModalType.MoreInfo)} className="bg-[#F2FA02] flex items-center gap-0">
          <img src="/assets/svg/banana-pixelated.svg" alt="Banana" className="w-7 h-7 ml-1" />
          <div
            className="flex flex-col py-1 pr-2 justify-center items-center font-bold text-[15px] gap-0 "
      ...
      <div className="flex gap-4">
        <OldButtonV2
          onClick={() => openModal(ModalType.Leaderboards)}
          className="bg-[#4F4F4F] w-[114px] h-10 grid place-items-center"
          title="Leaderboards"
        >
          <img src="/assets/icons/chart.png" alt="Leaderboards" className="w-5 h-5" />
        </OldButtonV2>
        <OldButtonV2
          onClick={() => openModal(ModalType.SearchCoins)}
          className="bg-[#4F4F4F] w-[114px] h-10 grid place-items-center"
          title="Search coins"
        >
        ...
        <OldButtonV2
          onClick={() => openModal(ModalType.FavouriteCoins)}
          className="bg-[#4F4F4F] w-[114px] h-10 grid place-items-center"
        ...
        {Features.TELESWAP_MODAL && (
          <OldButtonV2
            onClick={() => openModal(ModalType.Teleswap)}
            className="bg-[#4F4F4F] w-[114px] h-10 grid place-items-center"
            title="Swap tokens"
          >
